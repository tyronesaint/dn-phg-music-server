...
export class LyricService {
  ...
  async getPic(source: string, songId: string): Promise<string | null> {
    log.info("开始获取封面, source:", source, "songId:", songId);

    try {
      // 根据不同的平台使用不同的API获取封面
      switch (source) {
        case "wy":
          return await this.getWyPic(songId);
        case "tx":
          return await this.getTxPic(songId);
        case "kg":
          return await this.getKgPic(songId);
        case "kw":
          return await this.getKwPic(songId);
        case "mg":
          return await this.getMgPic(songId);
        default:
          throw new Error(`不支持的封面源: ${source}`);
      }
    } catch (error: any) {
      log.error("获取封面失败:", error.message);
      return null;
    }
  }

  // 网易云音乐封面
  private async getWyPic(songId: string): Promise<string> {
    ...
    throw new Error("未找到封面");
  }

  // QQ音乐封面
  private async getTxPic(songId: string): Promise<string> {
    const url = `https://u.y.qq.com/cgi-bin/musicu.fcg`;
    const body = {
      comm: { ct: '19', cv: '1859', uin: '0' },
      req: {
        module: 'music.pfSongDetailSvr',
        method: 'GetSongDetail',
        param: { song_id: songId, song_mid: songId }
      }
    };
    const response = await fetch(url, {
      method: 'POST',
    ...
    const data = await response.json();
    if (data.req && data.req.data && data.req.data.track_info) {
      return data.req.data.track_info.album.pmid;
    }
    throw new Error("未找到封面");
  }

  // 酷狗封面
  private async getKgPic(hash: string): Promise<string> {
    const url = `https://www.kugou.com/yy/index.php?r=play/getdata&hash=${hash}`;
    const response = await fetch(url);
    const data = await response.json();
  ...
  // 酷我封面
  private async getKwPic(songId: string): Promise<string> {
    const url = `http://artistpicserver.kuwo.cn/pic.web?songid=${songId}`;
    return url;
  }

  // 咪咕封面
  private async getMgPic(songId: string): Promise<string> {
    const url = `https://music.migu.cn/v3/api/song/v2/detail?songId=${songId}`;
    const response = await fetch(url);
    const data = await response.json();
    if (data.songs && data.songs[0] && data.songs[0].imgItems && data.songs[0].imgItems.length > 0) {
      return data.songs[0].imgItems[0].img;
    }
    throw new Error("未找到封面");
  }
}
