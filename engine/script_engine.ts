...
export class ScriptEngine {
  ...
  async search(scriptId: string, keyword: string, page = 1, limit = 20): Promise<any> {
    console.log('\n========== [ScriptEngine] search 开始 ==========');
    console.log('[ScriptEngine] scriptId:', scriptId);
    console.log('[ScriptEngine] keyword:', keyword);

    const sandbox = this.sandboxes.get(scriptId);
    
    if (!sandbox) {
      console.error('[ScriptEngine] 抛出异常: Script not found');
      console.log('========== [ScriptEngine] search 结束 ==========\n');
      throw new Error(`Script not found: ${scriptId}`);
    }

    try {
      const result = await sandbox.search(keyword, page, limit);
      console.log('[ScriptEngine] search 成功');
      console.log('========== [ScriptEngine] search 结束 ==========\n');
      return result;
    } catch (error: any) {
      console.error('[ScriptEngine] search 失败:', error.message);
      console.log('========== [ScriptEngine] search 结束 ==========\n');
      throw error;
    }
  }
